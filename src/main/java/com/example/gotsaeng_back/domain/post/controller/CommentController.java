...
public class CommentController {
    ...
    public CustomResponse<Post> createComment(@PathVariable Long postId, @RequestBody CreateCommentDTO commentDTO, @RequestHeader("Authorization") String token) {
        commentService.save(commentDTO, token, postId);
        return new CustomResponse<>(true, "댓글 작성 성공");
    }

    //댓글 삭제
    @DeleteMapping("/delete/{commentId}")
    public CustomResponse deleteComment(@PathVariable("commentId") Long commentId, @RequestHeader("Authorization") String token) {
        return commentService.deleteById(commentId, token);
    }

    //댓글 수정
    @PostMapping("/update/{commentId}")
    public CustomResponse updateComment(@PathVariable("commentId") Long commentId, @RequestBody UpdateCommentDTO commentDTO, @RequestHeader("Authorization") String token){
